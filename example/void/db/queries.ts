import { and, db as runtimeDb, eq, gt, sql } from 'void/db';
import {
  comment,
  event,
  eventAttendee,
...
type InsertDatabase = Pick<typeof runtimeDb, 'insert'>;
...
export const createPostRecord = async (
  ...
  database: InsertDatabase = runtimeDb,
) => {
  ...
  if (tagIds.length) {
    await database.insert(postToTag).values(tagIds.map((tagId) => ({ postId: created.id, tagId })));
  }
...
export const deleteCommentRecord = async (id: string) => {
  await runtimeDb.delete(comment).where(eq(comment.id, id));
};
...
export const createEventRecord = async (
  ...
  if (attendees.length) {
    await database.insert(eventAttendee).values(

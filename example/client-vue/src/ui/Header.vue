...
<template>
  ...
  >
    <div
      class="max-w-8xl relative container mx-auto flex h-16 items-center justify-between gap-4 px-6 lg:px-8"
    >
      <a href="/" @click.prevent="goHomeOrBack">
        <div class="flex items-center gap-3">
          <div
            class="squircle relative flex h-10 w-10 items-center justify-center bg-linear-to-br from-gray-100 to-gray-200 text-white shadow-lg shadow-gray-500/20 transition duration-250 hover:scale-110 active:scale-90"
          >
            <svg
              v-if="path === '/'"
            ...
            <ChevronLeft v-else class="h-6 w-6 text-gray-500" />
          </div>
          <div class="opacity-100 transition-opacity duration-250 hover:opacity-70">
            <span
      ...
      <div class="flex items-center">
        <Button size="sm" variant="ghost" @click="visit('/search')">
          <Search class="h-4 w-4" />
          <span class="hidden sm:inline">Search</span>
        </Button>
        <div class="flex w-9 justify-end sm:w-24">
          <Button v-if="session.data" class="w-full" size="sm" variant="ghost" :action="signOut">
          ...
          <div v-else aria-hidden class="h-9 w-full" />
        </div>
      </div>
    </div>
  </header>

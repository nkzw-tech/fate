...
import { z } from 'zod';
import { db, eq, like } from '../../db/db.ts';
import {
  type CommentItem,
  type PostItem,
  createCommentRecord,
...
const isDevelopment = Boolean(
  (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env?.DEV,
);
export const fateLive = createCloudflareFateLive();
export const { live } = fateLive;

type CloudflareAdapterContext = {
  env?: {
...
export const fateServer = createFateServer({
  context: ({ adapterContext, request }) =>
    createContext({
      request,
      secret: (adapterContext as CloudflareAdapterContext | undefined)?.env?.BETTER_AUTH_SECRET,
    }),
  lists: {
    commentSearch: {
